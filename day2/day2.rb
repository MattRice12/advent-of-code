...
def decode(line)
  ...
  [low.to_i, high.to_i, letter, password]
end


def old_password_count(data)
  valid_count = 0

  data.each do |line|
    low, high, letter, password = decode(line)
    count = password.count(letter)
    valid_count += 1 if count >= low && count <= high
  end

  puts valid_count
end

def new_password_count(data)
  valid_count = 0

  data.each do |line|
    first, last, letter, password = decode(line)
    result = []
    result.push(letter) if password[first-1] == letter
    result.push(letter) if password[last-1] == letter
    valid_count += 1 if result.length == 1
  end

  puts valid_count
end

new_password_count(data)